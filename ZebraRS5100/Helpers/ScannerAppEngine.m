...
@property (atomic,retain) UIAlertController *m_lastAlertView;
...
-(id)init
{
    ...
    if (self != nil)
    {
        ...
        self.m_lastAlertView = [[UIAlertController alloc] init];
...
- (void)showMessageBox:(NSString*)message
{
    zt_ScannerDemoAppDelegate *appDelegate = (zt_ScannerDemoAppDelegate *)[[UIApplication sharedApplication] delegate];
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (appDelegate.window.rootViewController.presentedViewController == self.m_lastAlertView)
            {
                [self.m_lastAlertView dismissViewControllerAnimated:FALSE completion:nil];
            ...
            [appDelegate.window.rootViewController presentViewController:self.m_lastAlertView animated:YES completion:nil];
        });
...
- (int)showBackgroundNotification:(NSString *)text aDictionary:(NSDictionary*)param_dict
{
    ...
    if ([[UIApplication sharedApplication] applicationState] != UIApplicationStateActive)
    {
        ...
        if (center){
            ...
            [content setTitle:ZT_SCANNER_APP_NAME];
            NSTimeInterval timeStampValue = [[NSDate date] timeIntervalSince1970];
            NSString *identifier =[NSString stringWithFormat:SCANNER_APP_ENGINE_NOTIFICATION_ID_FORMAT,ZT_SCANNER_NOTIFICATION_IDENTITY,timeStampValue];
...
//Get active scanners list from the SDK
- (NSArray*)getActiveScannersList
{
    NSMutableArray *activeScanners = [[[NSMutableArray alloc] init] autorelease];
    
    [m_DcsSdkApi sbtGetActiveScannersList:&activeScanners];
...
- (void)enableBluetoothScannerDiscovery:(BOOL)enable
{
    if (m_DcsSdkApi != nil)
    {
        [m_DcsSdkApi sbtEnableBluetoothScannerDiscovery:enable];
    }
}
...
- (UIImage*) sbtSTCPairingBarcode:(BARCODE_TYPE)barcodeType withComProtocol:(STC_COM_PROTOCOL)comProtocol withSetDefaultStatus:(SETDEFAULT_STATUS)setDefaultsStatus withImageFrame:(CGRect)imageFrame
{
    return [m_DcsSdkApi sbtGetPairingBarcode:barcodeType withComProtocol:comProtocol withSetDefaultStatus:setDefaultsStatus  withImageFrame:imageFrame];
}
...
- (void)sbtEventScannerDisappeared:(int)scannerID
{
    ...
    if ([self isInBackgroundMode] == YES)
    {
        ...
        ///Prevent disappeared notification if virtual tether host feedback enabled
        if (!isHostFeedbackEnable){
            if ([[NSUserDefaults standardUserDefaults] boolForKey:ZT_SETTING_NOTIFICATION_AVAILABLE] == YES){
        ...
        for (id<IScannerAppEngineDevConnectionsDelegate> delegate in [[m_DevConnectionsDelegates copy] autorelease])
        {
            if (delegate != nil)
            {
                ...
                if (result == YES)
                {
                     ...
                     */
                    notificaton_processed = NO;
                    // Re-connection established successfully.
                    // Reset auto reconnect to false for all scanners and set true for connected scanner.
                    [self setAutoReconnectOptionAfterScannerConnected:[activeScanner getScannerID]];
                }
            }
        }
...
- (void)sbtEventCommunicationSessionTerminated:(int)scannerID
{
    ...
    if ([self isInBackgroundMode] == YES)
    {
        ...
        BOOL isHostFeedbackEnable = (BOOL)[stdUserDefaults boolForKey:ZT_VIRTUAL_TETHER_ENABLE_HOST_FEEDBACK];
        [[ConnectionManager sharedConnectionManager] setVirtualTetherUI:NO];
        ///Prevent disconnected notification if virtual tether host feedback enabled
        if (isHostFeedbackEnable){///NSUserDefaults returns NO for isHostFeedbackEnable and isHostVibrateEnable if the key doesn't exist.
            NSDictionary *notif_dict = [NSDictionary dictionaryWithObject:[NSNumber numberWithInt:scannerID] forKey:ZT_NOTIFICATION_KEY_SCANNER_ID];
            NSString *virtualTetherNotification = [NSString stringWithFormat:@"%@",VIRTUAL_TETHER_NOTIFICATION_TITLE];
            [self showBackgroundNotification:virtualTetherNotification aDictionary:notif_dict];
        }else{
            if ([[NSUserDefaults standardUserDefaults] boolForKey:ZT_SETTING_NOTIFICATION_ACTIVE] == YES)
            {
    ...
    if ([self isInBackgroundMode] == NO)
    {
        if (notificaton_processed == NO)
        {
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                NSUserDefaults *stdUserDefaults = [NSUserDefaults standardUserDefaults];
                ///Prevent disconnected popup message if Host Feedback enabled
                BOOL isHostFeedbackEnable = (BOOL)[stdUserDefaults boolForKey:ZT_VIRTUAL_TETHER_ENABLE_HOST_FEEDBACK];
                if (!isHostFeedbackEnable){
